...
class UserController extends BackendController
{
	...
	public function changePassAction()
	{
		...
		$this->_arrParam['form']['password'] = $this->randomString(8);
		$source = $this->_arrParam['form'];
		...
		if (!empty($source['token'])) {
			...
			if ($this->_validate->isValid() == false) {
				...
				$this->_model->editPassword($this->_arrParam, array('task' => 'edit'));
	...
	{
		$change = $this->_model->changeGroupName($this->_arrParam);
		echo json_encode($change);
	}

	public function changeOrderingAction()
	{
		$modified = $this->_model->changeOrdering($this->_arrParam);
